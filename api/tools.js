...
async function handleIpGeo(req, res) {
  ...
  const u = await auth(req, res); if (!u) return;
  const t = Date.now();
  const r = await fetch(`http://ip-api.com/json/${ip}?fields=status,message,country,countryCode,region,regionName,city,zip,lat,lon,timezone,isp,org,as,mobile,proxy,hosting`);
  const d = await r.json();
  if (d.status === 'fail') return res.status(400).json({ error: d.message, ip });
  await log(u.uid, '/api/ip-geo', ip);
  return res.json({ ip, country: d.country, country_code: d.countryCode, region: d.regionName, city: d.city, zip: d.zip, latitude: d.lat, longitude: d.lon, timezone: d.timezone, isp: d.isp, is_mobile: d.mobile, is_proxy: d.proxy, is_hosting: d.hosting, credits_remaining: u.credits - 1, latency_ms: Date.now() - t });
}
...
async function handlePlaceholder(req, res) {
  ...
  const lb = (ct || `${W} × ${H}`).replace(/&/g, '&amp;').replace(/</g, '&lt;');
  const u = await auth(req, res); if (!u) return;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="100%" height="100%" fill="#${bg}"/><text x="50%" y="50%" dominant-baseline="central" text-anchor="middle" font-family="system-ui,sans-serif" font-size="${fs}" fill="#${color}">${lb}</text></svg>`;

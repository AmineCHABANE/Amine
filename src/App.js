...
function BannerAd({ format = 'rectangle', plan }) {
  const id = `banner-${format}-${Math.random().toString(36).substring(7)}`;
  ...
  return (
    <div className="ad-banner">
      ...
      <div id={id} style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none' }} />
    </div>
  );
}
...
function Dashboard({ session }) {
  ...
  const testApi = async () => {
    ...
    try {
      ...
      // Show interstitial every 3rd request (free users only)
